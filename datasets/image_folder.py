...
class ImageFolder(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache == 'none':
            return self.img_process(x), filename
        elif self.cache == 'in_memory':
            return x, filename
...
class PairedImageFolders(Dataset):
    ...
    def __getitem__(self, idx):
        img1, filename1 = self.dataset_1[idx]
        img2, filename2 = self.dataset_2[idx]
#        assert filename1 == filename2, f"Filenames don't match: {filename1} vs {filename2}"
        
        return img1, img2, filename1