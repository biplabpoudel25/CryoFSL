...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None,
        ...
        elif self.loss_mode == 'diceCE':
            from monai.losses import DiceCELoss
            self.criterionBCE = DiceCELoss(to_onehot_y=True, softmax=True)
        
        elif self.loss_mode == 'dice_bce':
            self.criterionBCE = torch.nn.BCEWithLogitsLoss()
...
#        bs = 2
    ...
    def backward_G(self):
        ...
        if self.loss_mode == 'iou':
            ...
            self.loss_G += DiceLoss()(nn.Sigmoid()(self.pred_mask), self.gt_mask)

        self.loss_G.backward()

#    def backward_G(self):
#        """Calculate loss for the generator but don't apply backward() immediately"""
#        self.loss_G = self.criterionBCE(self.pred_mask, self.gt_mask)
#        if self.loss_mode == 'iou':
#            self.loss_G += _iou_loss(self.pred_mask, self.gt_mask)
#        elif self.loss_mode == 'dice_bce':
#            self.loss_G += DiceLoss()(nn.Sigmoid()(self.pred_mask), self.gt_mask)
#        
#        # Only call backward() if not using mixed precision
#        # (We'll check this in optimize_parameters)
#        if not hasattr(self, '_using_mixed_precision') or not self._using_mixed_precision:
#            self.loss_G.backward()
#        
#        # Return the loss for use with mixed precision
#        return self.loss_G
...
#        # If scaler is provided, use mixed precision training
#        if scaler is not None:
#            # Set flag to prevent backward() in backward_G
#            self._using_mixed_precision = True
#        
#            with autocast():
#                self.forward()
#                loss = self.backward_G()  # Just calculate loss, don't call backward
#            
...
#            
#            # Reset flag
#            self._using_mixed_precision = False
#            
#        else:
#            # Original behavior without mixed precision
#            self.forward()
